...
export function VoiceLoop() {
  ...
  const handlePhotoCapture = useCallback(
    async (imageData: string) => {
      ...
      // 90s cooldown so the AI's response about the photo doesn't re-trigger camera
  ...
  const handlePhotoCancel = () => {
    // Short cooldown so cancelling doesn't immediately re-trigger the same phrase
